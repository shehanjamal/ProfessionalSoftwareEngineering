...
def generate_itinerary(destination, duration, interests="general"):
    """
    Generates a travel itinerary based on destination, duration, and interests.
    """
    model = genrativeai.GenerativeModel('models/gemini-pro-latest') # This line requires a configured API key
    ...
    try:
        ...
        # Check for candidates and return the text
        if response.candidates:
            # Access the text from the first part of the first candidate
            return response.candidates[0].content.parts[0].text
        else:
            # This can happen if the AI generates no suitable content (e.g., due to safety filters)
        ...
        # This will catch errors related to the API key, network, or model issues
